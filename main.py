...
def index():
    ...
    cards = Card.query.filter_by(user_email=session['user.email']).order_by(Card.id).all()
...
def form_create():
    if request.method == 'POST':
        ...
        card = Card(title=title, subtitle=subtitle, text=text, user_email=session['user.email'])
